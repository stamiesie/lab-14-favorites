...
app.get('/api/favorites', async (req, res) => {
  try {
    const data = await client.query('SELECT * from favorites WHERE owner_id=$1',
      [
        req.userId
      ]);
...
app.post('/api/favorites', async (req, res) => {
  try {
    ...
    (title, tagline, budget, release_date, poster, movie_api_id)
...
app.delete('/api/favorites/:id', async (req, res) => {
  try {
    ...
    const data = await client.query(`DELETE favorites 
